...
def generate_search_queries_from_context(
    client: OpenAI,
    model: str,
    context: str,
    max_queries: int = 6,
) -> list[str]:
    """Ask the LLM to propose Korean web search queries tailored to the candidate."""
    prompt = (
        "아래 자기소개서/포트폴리오/면접 요약을 보고, 경력 개발/프로그램/캠프/강의/기술 트렌드/면접 준비에 도움이 될 "
        "한국어 웹 검색 쿼리 6개 이하를 JSON으로 반환하세요. "
        "각 쿼리는 12단어 이하, 서비스/기술명/직무명이 보이면 포함하세요. "
        "반드시 JSON 객체 형태로, 키 'queries'에 문자열 배열만 포함하세요."
    )
    try:
        raw = get_text_response(
            ...
            model=model,
            messages=[
                {"role": "system", "content": "검색 키워드를 설계하는 리서치 어시스턴트입니다."},
    ...
    if not queries:
        queries = [
            "백엔드 개발자 커리어 부트캠프 추천",
            "최신 백엔드 아키텍처 사례 인터뷰 질문",
            "클라우드 네이티브 백엔드 프로젝트 예제",
            "마이크로서비스 설계 코드리뷰 스터디",
            "대규모 트래픽 처리 면접 대비 자료",
        ]
...
def run_duckduckgo_search(
    ...
    """Execute DuckDuckGo searches and return both structured and plain text snippets."""
    if not queries:
        return [], ""
    ...
    try:
        ...
        raise RuntimeError(
            "duckduckgo-search가 설치되어야 웹 서치를 수행할 수 있습니다. `pip install duckduckgo-search`를 실행하세요."
        ) from exc
    ...
    lines = []
    for item in results:
        line = f"[{item['query']}] {item['title']}: {item['snippet']}"
        if item["link"]:
...
class WorkflowState(TypedDict, total=False):
    ...
    portfolio_search_queries: Optional[list[str]]
    portfolio_search_snippets: Optional[str]
    portfolio_search_results_path: Optional[str]
...
def portfolio_development_node(state: WorkflowState) -> WorkflowState:
    ...
    # Build search queries from the candidate's materials and fetch web snippets
    search_queries: list[str] = []
    search_results_text = ""
    search_results_path: Optional[Path] = output_dir / "Portfolio_Search_Results.txt"
    try:
        search_queries = generate_search_queries_from_context(
            client,
            model="gpt-5.1",
            context=(
                f"{introduce_text}\n\n{portfolio_text}\n\n"
        ...
        _, search_results_text = run_duckduckgo_search(
            queries=search_queries,
            per_query=3,
            max_chars=2400,
        )
        if search_results_text:
            search_results_path.write_text(search_results_text, encoding="utf-8")
    except Exception as exc:
        search_results_text = f"(검색 실패) {exc}"
...
<검색에 사용한 쿼리>
{', '.join(search_queries) if search_queries else "(생성 실패)"}

<실제 웹 검색 결과 요약>
{search_results_text or "(검색 결과 없음)"}
"""

    prompt = f"""위 문서를 바탕으로 다음을 모두 포함한 Markdown 보고서를 작성하세요.

요구사항:
- 위에 제공된 실제 웹 검색 결과 요약을 근거로, 최신 정보를 조사하듯이 추천 출처를 간단히 제시하세요.
...
"""
    ...
    try:
        ...
        return {
            ...
            "portfolio_search_queries": search_queries,
            "portfolio_search_snippets": search_results_text,
            "portfolio_search_results_path": str(search_results_path) if search_results_path else None,
        }
    except Exception as e:
...
def insight_dashboard_node(state: WorkflowState) -> WorkflowState:
    ...
    search_snippets = state.get("portfolio_search_snippets") or ""
    search_queries = state.get("portfolio_search_queries") or []
    ...
    prompt = f"""당신은 프론트엔드 디자이너 겸 커리어 코치 요약가입니다.
단일 HTML 페이지를 만들어, 면접 평가/포트폴리오 리포트/웹 검색 인사이트를 합쳐
사용자 맞춤 로드맵을 제시하세요.

구성 요구:
- <!DOCTYPE html>부터 시작, 단일 컬럼, 노션 느낌. 배경/패널 #F5F5F5, 하이라이트 배경 #0A193D, 포인트 텍스트 #0A091A.
- 토글(details/summary) 3개 이상: (1) Debate 최종 요약, (2) 포트폴리오/답변 통합 요약, (3) 검색 인사이트 및 추천 링크, (4) 빠른 액션 등.
- 검색 결과와 보고서에 등장하는 URL을 pill/button 형태로 나열(중복 제거, 최대 10개). 링크 출처 표시.
- 두 개의 로드맵 카드를 반드시 포함:
  * 1개월 단기 로드맵: 주차별(week-by-week) 3~4단계, 각 단계에 목표·실행 액션·필요 자료/링크를 기입.
  * 1년 장기 로드맵: 분기 또는 2개월 단위 4~6단계, 각 단계에 목표·측정 가능한 마일스톤·추천 활동/자료/링크를 기입.
- 로드맵 내용은 Debate/포트폴리오 리포트, 면접 답변, 그리고 아래 제공된 실제 웹 검색 결과 요약을 근거로 작성.
- 액션 리스트는 bullet 6~8개로 압축. 가독성 높은 폰트(Inter, Pretendard 우선), 라운드 카드, subtle shadow 유지.
- JS 최소화(토글 기본 사용).
...
--- 사용한 검색 쿼리 ---
{', '.join(search_queries) if search_queries else "(없음)"}
"""
    ...
    html = get_text_response(
        ...
        messages=[
            {
                "role": "system",
                "content": "당신은 노션 느낌의 단일 컬럼 HTML 대시보드를 만드는 프론트엔드 디자이너이자 커리어 코치입니다. 토글/요약/링크를 컴팩트하게 배치하고, 1개월/1년 로드맵을 구체적으로 설계하세요.",
            },
            {"role": "user", "content": prompt},
        ],
        temperature=0.4,
...
def debate_node(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        # Round 4: Objective Criteria Clarification
        debate_log.append("=" * 80)
        debate_log.append("ROUND 4: OBJECTIVE CRITERIA CLARIFICATION")
        debate_log.append("=" * 80)
        debate_log.append("")

        attitude_criteria = get_text_response(
            client,
            model="gpt-5.1",
            messages=[
                {"role": "system", "content": "당신은 면접 태도 평가 전문가입니다. 자신의 평가 기준을 객관적으로 명시하세요."},
                {"role": "user", "content": f"당신의 초기 평가:\n{attitude_eval}\n\n당신의 기준을 명확히 3-4개로 제시하세요."},
            ],
            temperature=0.6,
        )
        qa_criteria = get_text_response(
            client,
            model="gpt-5.1",
            messages=[
                {"role": "system", "content": "당신은 질문-답변 연관성 평가 전문가입니다. 자신의 평가 기준을 객관적으로 명시하세요."},
                {"role": "user", "content": f"당신의 초기 평가:\n{qa_eval}\n\n당신의 기준을 명확히 3-4개로 제시하세요."},
            ],
            temperature=0.6,
        )
        consistency_criteria = get_text_response(
            client,
            model="gpt-5.1",
            messages=[
                {"role": "system", "content": "당신은 일관성 평가 전문가입니다. 자신의 평가 기준을 객관적으로 명시하세요."},
                {"role": "user", "content": f"당신의 초기 평가:\n{consistency_eval}\n\n당신의 기준을 명확히 3-4개로 제시하세요."},
            ],
            temperature=0.6,
        )
        debate_log.append("[Attitude Agent - Criteria]")
        debate_log.append(attitude_criteria)
        debate_log.append("")
        debate_log.append("[QA Relevance Agent - Criteria]")
        debate_log.append(qa_criteria)
        debate_log.append("")
        debate_log.append("[Consistency Agent - Criteria]")
        debate_log.append(consistency_criteria)
        debate_log.append("")

        # Round 5: Evidence & Gaps Challenge
        debate_log.append("=" * 80)
        debate_log.append("ROUND 5: EVIDENCE & GAPS CHALLENGE")
        debate_log.append("=" * 80)
        debate_log.append("")

        attitude_gap = get_text_response(
            client,
            model="gpt-5.1",
            messages=[
                {"role": "system", "content": "당신은 면접 태도 평가 전문가입니다. 다른 에이전트 평가의 증거/갭을 짧게 지적하세요."},
                {"role": "user", "content": f"QA 평가: {qa_eval}\n일관성 평가: {consistency_eval}\n증거 부족이나 보완 필요 지점을 2-3줄로 지적하세요."},
            ],
            temperature=0.6,
        )
        qa_gap = get_text_response(
            client,
            model="gpt-5.1",
            messages=[
                {"role": "system", "content": "당신은 질문-답변 연관성 평가 전문가입니다. 다른 에이전트 평가의 증거/갭을 짧게 지적하세요."},
                {"role": "user", "content": f"태도 평가: {attitude_eval}\n일관성 평가: {consistency_eval}\n증거 부족이나 보완 필요 지점을 2-3줄로 지적하세요."},
            ],
            temperature=0.6,
        )
        consistency_gap = get_text_response(
            client,
            model="gpt-5.1",
            messages=[
                {"role": "system", "content": "당신은 일관성 평가 전문가입니다. 다른 에이전트 평가의 증거/갭을 짧게 지적하세요."},
                {"role": "user", "content": f"태도 평가: {attitude_eval}\nQA 평가: {qa_eval}\n증거 부족이나 보완 필요 지점을 2-3줄로 지적하세요."},
            ],
            temperature=0.6,
        )
        debate_log.append("[Attitude Agent - Evidence Gaps]")
        debate_log.append(attitude_gap)
        debate_log.append("")
        debate_log.append("[QA Relevance Agent - Evidence Gaps]")
        debate_log.append(qa_gap)
        debate_log.append("")
        debate_log.append("[Consistency Agent - Evidence Gaps]")
        debate_log.append(consistency_gap)
        debate_log.append("")

        # Round 6: Closing Positions
        debate_log.append("=" * 80)
        debate_log.append("ROUND 6: CLOSING POSITIONS")
        debate_log.append("=" * 80)
        debate_log.append("")

        attitude_closing = get_text_response(
            client,
            model="gpt-5.1",
            messages=[
                {"role": "system", "content": "당신은 면접 태도 평가 전문가입니다. 최종 입장을 2-3문장으로 명확히 하세요."},
                {"role": "user", "content": f"초기 평가: {attitude_eval}\n교차검토: {attitude_to_qa}\n반박: {attitude_rebuttal}\n기준: {attitude_criteria}\n증거/갭 지적: {attitude_gap}\n\n최종 입장을 2-3문장으로 압축하세요."},
            ],
            temperature=0.5,
        )
        qa_closing = get_text_response(
            client,
            model="gpt-5.1",
            messages=[
                {"role": "system", "content": "당신은 질문-답변 연관성 평가 전문가입니다. 최종 입장을 2-3문장으로 명확히 하세요."},
                {"role": "user", "content": f"초기 평가: {qa_eval}\n교차검토: {qa_to_consistency}\n반박: {qa_rebuttal}\n기준: {qa_criteria}\n증거/갭 지적: {qa_gap}\n\n최종 입장을 2-3문장으로 압축하세요."},
            ],
            temperature=0.5,
        )
        consistency_closing = get_text_response(
            client,
            model="gpt-5.1",
            messages=[
                {"role": "system", "content": "당신은 일관성 평가 전문가입니다. 최종 입장을 2-3문장으로 명확히 하세요."},
                {"role": "user", "content": f"초기 평가: {consistency_eval}\n교차검토: {consistency_to_attitude}\n반박: {consistency_rebuttal}\n기준: {consistency_criteria}\n증거/갭 지적: {consistency_gap}\n\n최종 입장을 2-3문장으로 압축하세요."},
            ],
            temperature=0.5,
        )
        debate_log.append("[Attitude Agent - Closing]")
        debate_log.append(attitude_closing)
        debate_log.append("")
        debate_log.append("[QA Relevance Agent - Closing]")
        debate_log.append(qa_closing)
        debate_log.append("")
        debate_log.append("[Consistency Agent - Closing]")
        debate_log.append(consistency_closing)
        debate_log.append("")

        # Round 7: Final Consensus
        debate_log.append("=" * 80)
        debate_log.append("ROUND 7: FINAL CONSENSUS")
        debate_log.append("=" * 80)
        debate_log.append("")
...
- Attitude: {attitude_criteria}
- QA: {qa_criteria}
- Consistency: {consistency_criteria}

**Evidence/Gaps Challenge:**
- Attitude: {attitude_gap}
- QA: {qa_gap}
- Consistency: {consistency_gap}

**Closing Positions:**
- Attitude: {attitude_closing}
- QA: {qa_closing}
- Consistency: {consistency_closing}
